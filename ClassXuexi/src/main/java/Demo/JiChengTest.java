...
class Cat extends JiChengTest{
 ...
 /*
   ...
   2.子类中的方法名必须与父类中的方法名一致，参数列表要一致，返回类型是相同的返回类型或者父类方法返回类型的子类类型。
   4.子类继承父类时，不能继承父类的构造方法/静态方法，只可以重写父类中的实例方法（没有加static的方法）。
   5.子类继承父类后，子类中可以直接访问父类的属性。
   6.当重载以后，在调用时，根据继承规则找相匹配的执行。
   7.重写以后，父类中的实例方法依然存在，只是不执行了，如果不存在就达不到重写的效果。
   ---------------------------------------------------------------------
   构造方法的重载是
   在同一个类中，方法名必须相同，但是参数列表必须不同（个数，顺序,类型不同)，与返回值类型无关。
   *
  */
    @Override
    //重写父类的方法
